...
class google_model:
    def __init__(self, sess, name, num_classes):
        self.sess = sess
        self.name = name
    ...
    def get_inception_layer(self, inputs, conv11_size, conv33_11_size, conv33_size, conv55_11_size, conv55_size, pool11_size):
        with tf.variable_scope("conv_1x1"):
            conv11 = layers.conv2d(inputs, conv11_size, [1, 1], scope='conv_1x1')
        with tf.variable_scope("conv_3x3"):
            conv33_11 = layers.conv2d(inputs, conv33_11_size, [1, 1])
            conv33 = layers.conv2d( conv33_11, conv33_size, [3, 3], scope='conv_3x3')
        with tf.variable_scope("conv_5x5"):
            conv55_11 = layers.conv2d(inputs, conv55_11_size, [1, 1])
            conv55 = layers.conv2d( conv55_11, conv55_size, [5, 5], scope='conv_5x5')
        with tf.variable_scope("pool_proj"):
            pool_proj = layers.max_pool2d(inputs, [3, 3], stride=1, scope='pool_proj')
    ...
    def googlenet(self, dropout_keep_prob=0.4, scope=''):
        '''
        ...
        with tf.name_scope(scope, "googlenet", [x_img]):
            with ops.arg_scope( [ layers.max_pool2d ], padding = 'SAME' ):
                self.end_points['conv0'] = layers.conv2d(x_img, 64, [3, 3], scope='conv0')
                print(self.end_points['conv0'])

                self.end_points['pool0'] = layers.max_pool2d(self.end_points['conv0'], [2, 2], stride=2,  scope='pool0')
                ...
                self.end_points['pool1'] = layers.max_pool2d(self.end_points['conv1_b'], [2, 2], stride=2, scope='pool1')
                ...
                print(self.end_points['inception_3a'])
                ...
                self.end_points['pool2'] = layers.max_pool2d(self.end_points['inception_3b'], [2, 2], stride=2, scope='pool2')
                ...
                print(self.end_points['inception_4a'])
                ...
                print(self.end_points['inception_4b'])
                ...
                print(self.end_points['inception_4c'])
                ...
                print(self.end_points['inception_4d'])
                ...
                self.end_points['pool3'] = layers.max_pool2d(self.end_points['inception_4e'], [2, 2], stride=2, scope='pool3')
                ...
                print(self.end_points['inception_5a'])
                ...
                print(self.end_points['inception_5b'])

                self.end_points['pool4'] = layers.avg_pool2d(self.end_points['inception_5b'], [2, 2], stride=2, scope='pool4')
                print(self.end_points['pool4'])

                initializer = tf.contrib.layers.xavier_initializer()

                self.end_points['flat'] = tf.contrib.layers.flatten(self.end_points['pool4'])
                print(self.end_points['flat'])
                ...
                print(self.end_points['dropout'])

                self.end_points['logits'] = tf.layers.dense(inputs=self.end_points['dropout'], units=self.num_classes)
                ...
                tf.summary.scalar('cost', self.end_points['cost'])
...
y_train_one_hot = tf.squeeze(tf.one_hot(y_train, case_num), axis=1)
y_test_one_hot = tf.squeeze(tf.one_hot(y_test, case_num), axis=1)

sess = tf.Session()

model = google_model(sess, "sex", case_num)
