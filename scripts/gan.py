...
with tf.Session() as sess:
    ...
    for i in range(num_epoch):
        ...
        if (i % 100 == 0)and(i > 0):
            ...
            if i % 500 == 0:
                x = range(100)
                plt.plot(x, g_loss_list[(i-1)//100 * 100: i//100 * 100], label='g loss')
                plt.plot(x, d_loss_list[(i-1)//100 * 100: i//100 * 100], label='d loss')
                plt.xlabel("resent epochs")
                plt.ylabel("loss")
                plt.ylim(0, 7)
                plt.xlim(0, 100)
